...
log "✅ GitHub authenticated as: $GITHUB_USERNAME"
...
log "📁 Downloading sample app into hello-cloudbuild-app..."
...
log "🔧 Replacing region in YAML files..."
...
mv kubernetes.yaml.tpl kubernetes.yaml


log "📦 Initializing git repo for app..."
check git init
...
log "🔑 Generating SSH key and storing in Secret Manager..."
...
log "🔐 Assigning IAM roles to GCP service accounts..."
...
log "📁 Setting up hello-cloudbuild-env repo..."
...
log "🔧 Updating region in environment YAML files..."
for f in cloudbuild.yaml cloudbuild-delivery.yaml cloudbuild-trigger-cd.yaml kubernetes.yaml.tpl; do
    check sed -i "s/us-central1/$REGION/g" "$f"
done
...
echo
echo
log "🔗 Click this link to create a Cloud Build trigger:"
